...
Each cycle, the champion reads `.lathe/agents/champion.md` + the project snapshot, picks one stakeholder, uses the project as them, then picks one change. It commits a per-cycle goal file that the builder reads. The builder implements it. The verifier checks the work and writes a verdict:
