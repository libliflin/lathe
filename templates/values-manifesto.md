...
Each cycle has three phases. A **customer champion** picks one of the stakeholders and actually uses the project as them — running the commands, reading the output, hitting the friction, noticing the moments that feel good or hollow — then names the single change that would most improve the next encounter. A builder implements it. A verifier runs a rigorous check on the builder's work — did it actually meet the goal? Where could it be stronger? — and the builder and verifier loop until the verifier confirms the goal is met. CI gates every step.

The champion is not a separate simulator agent handing off a report. The champion is the role that used to be called "goal-setter" — same place in the loop, same file on disk (`.lathe/goal.md`), different posture. Instead of reading code analytically and guessing what would help a stakeholder, the champion inhabits a stakeholder, walks their journey, and lets lived experience drive the goal. This is what gives it the courage to name what's valuable and what's painful specifically, rather than hedging into safe polish work.
